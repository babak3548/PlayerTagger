...
CString gKey(L"{00000000-0000-0000-0000-000000000000}");
...
void CPlayer::SetFileType( SMM_OutputTypes ftInput )
{
	m_ftType = ftInput;
}
...
HRESULT CPlayer::SetIndex( CFilterWrapper *pDemuxer, CIndexEx *pIndex )
{
	HRESULT hr = S_FALSE;
	CheckPointer( pDemuxer, E_INVALIDARG );
	CheckPointer( pIndex, E_INVALIDARG );
	CComPtr<IIndexConfig> pIIndexConfig = NULL;
	hr = pDemuxer->QueryInterface( &pIIndexConfig );
	if( FAILED(hr) )
	{
		CExMsg err(_T("Querying IIndexConfig has been failed"), hr );
		err.MsgBox();
	}
	else
	{
		ASSERT(pIndex);
		IMediaIndex *pIMediaIndex = NULL; 
		pIMediaIndex = pIndex->GetFrameIndex();

		if( !pIMediaIndex )
		{
			CExMsg err(_T("Querying IMediaIndex has been failed"), E_FAIL);
			err.MsgBox();
		}
		else
			hr = pIIndexConfig->UseIndex( pIMediaIndex );
	}
	return hr;
}
HRESULT CPlayer::CreateFilters()
{
	...
	GUID CodecGUID;
	if( m_ftType == SMM_File_Type_MP4 )
		CodecGUID = parser.GetFilterCLSID( ProfileParser::Id_Demuxer, ProfileParser::prmCONTAINER, ProfileParser::cMP4 );
	else if( m_ftType == SMM_File_Type_MKV )
		CodecGUID = CLSID_SMM_MKV_SPLITTER;
	else
	...
	CodecGUID = parser.GetFilterCLSID( ProfileParser::Id_Decoder, ProfileParser::prmVES, ProfileParser::vesAVC);
	...
	//Set the index
	if( m_ftType != SMM_File_Type_MP4 && m_ftType != SMM_File_Type_MKV )
	{
		CComPtr<IIndexConfig> pIIndexConfig = NULL;
		m_pFilterDemuxer->QueryInterface( &pIIndexConfig );
		SetIndex( m_pFilterDemuxer, m_pIndex );
	}
	//Remove the logo from preview graph
	ActivateFilter(pBaseFilter);
...
HRESULT CPlayer::BuildGraph()
{
	...
	if( m_InputFile.data() )
	{
		...
		CFilterWrapper fSource;
		m_hrError = AddFilterSource( CLSID_AsyncReader, _T("Async Source"), GetWCHAR(m_InputFile.data(), wcsFName), &fSource );
		if( SUCCEEDED( m_hrError ) )
			m_hrError = p->Render( fSource.GetPin( PINDIR_OUTPUT ) );
		if( FAILED(m_hrError) )
			m_hrError = p->RenderFile(GetWCHAR(m_InputFile.data(), wcsFName), NULL);
	}
	else
		m_hrError = E_FAIL;
...
//This method should be applied to elecard filters and will disable the ELECARD logo
HRESULT CPlayer::ActivateFilter( IBaseFilter* filter )
{
	GUID Key_Guid;
	BSTR bstKey = gKey.AllocSysString();
	CLSIDFromString( bstKey, &Key_Guid );
	SysFreeString(bstKey);
	HRESULT hr = E_FAIL;
	IModuleConfig* IMC = NULL;
	hr = filter->QueryInterface(IID_IModuleConfig,(void**)&IMC );
	if (SUCCEEDED(hr))
	{
		hr = IMC->SetValue(&Key_Guid, NULL);
		hr = IMC->CommitChanges( NULL );
		IMC->Release();
	}
	return hr;
}
...
m_pConfigDmp(NULL),
m_rtListDuration(0),
m_OutFormat(SMM_File_Type_MPEG2_TS)
{
	m_pFilterMuxer = NULL;
}
...
HRESULT  CTrimGrpah::CreateFilters()
{
	...
	var.bstrVal = CString(g_ActionKey_Trimm_MKV_MP4_MOV_AVC).AllocSysString();
	...
	CComPtr<IClassFactory>  pClassFactory = NULL;
	CComPtr<IBaseFilter> pBaseFilter = NULL;
	CLSID CodecGUID = GUID_NULL;
	//Demuxer Instance
	ProfileParser & parser = ProfileParser::Instance(Default_Codecs_Profile);
	if( m_OutFormat == SMM_File_Type_MP4 )
		CodecGUID = parser.GetFilterCLSID( ProfileParser::Id_Demuxer, ProfileParser::prmCONTAINER, ProfileParser::cMP4 );
	else if( m_OutFormat == SMM_File_Type_MKV )
		CodecGUID = CLSID_SMM_MKV_SPLITTER;
	else
		CodecGUID = parser.GetFilterCLSID( ProfileParser::Id_Demuxer, ProfileParser::prmCONTAINER, ProfileParser::cMPEG2 );
	...
	if( FAILED(m_hrError) )
	{
		CExMsg err(_T("Couldn't create Elecard demuxer filter"), m_hrError);
		err.MsgBox();
		return m_hrError;
	}
	...
	//set index into demuxer in case of non mp4 files
	//or
	//tune up the demuxer if the file is mp4 file
	if( m_OutFormat == SMM_File_Type_MP4 )
	{
		// set annexB option because avc trimmer takes only annexB
		// set ADTS headers option off because Elecard mp4 multiplexer can't
		//write correct audio headers for aac stream with this option
		CComPtr<IModuleConfig> mc;
		m_pFilterDemuxer->QueryInterface(&mc);
		...
		if (mc)
		{
			...
			v.vt = VT_BOOL;
			v.boolVal = true;
			hr = mc->SetValue(&SMMP4_H264_ANNEXB,&v);
	...
	else if( m_OutFormat == SMM_File_Type_MKV )
	{
		...
		m_pFilterDemuxer->QueryInterface(&mc);
		{
			...
			v.vt = VT_BOOL;
			v.boolVal = true;
			hr = mc->SetValue(&SMMKV_H264_ANNEXB,&v);
			if( FAILED(hr) )return hr;
		}
		// set separate pins for splitter
		{
			VARIANT v;
			v.vt = VT_BOOL;
			v.boolVal = true;
			hr = mc->SetValue(&SMMKV_SEPARATE_PINS,&v);
			if( FAILED(hr) )return hr;
		}
	}
	else
		m_hrError = SetIndex( m_pFilterDemuxer, m_pIndex );
	...
	if( m_OutFormat == SMM_File_Type_MPEG2_TS || m_OutFormat == SMM_File_Type_MP4 || m_OutFormat == SMM_File_Type_MKV )
	{
		...
		//Muxer Instance
		if( m_OutFormat == SMM_File_Type_MP4 )
			CodecGUID = parser.GetFilterCLSID( ProfileParser::Id_Muxer, ProfileParser::prmCONTAINER, ProfileParser::cMP4 );
		else if( m_OutFormat == SMM_File_Type_MKV )
			CodecGUID = CLSID_SMM_MKV_MUXER;
		else
	...
	//Trimmer instance
	hr = AddFilter( CLSID_AVC_TRIM_FA, _T("Elecard-SolveigMM MPEG2 Trimmer FA"), &m_FilterTrimmer );
	if( FAILED( hr ) )
	{
	...
	var.bstrVal = NULL;
	ASSERT(m_pIndex);
	hr = (*m_pIndex)->get_IndexFileName( &var.bstrVal );
	...
	if(m_OutFormat != SMM_File_Type_MP4 && m_OutFormat != SMM_File_Type_MKV)
	{
		var.boolVal = TRUE;
		hr = pModuleConfig->SetValue( &SM2TFA_UseMPEG2Index, &var );
	}
	//Set the key guid to activate filters
	var.bstrVal = gKey.AllocSysString();
	hr = pModuleConfig->SetValue( &SMAT_3rdParty, &var );
	SysFreeString( var.bstrVal );
...
HRESULT  CTrimGrpah::QueryInterfaces()
{
	...
	else if( m_OutFormat == SMM_File_Type_MPEG2_VES )
	{
...
HRESULT  CTrimGrpah::BuildGraph()
{
	...
	if( m_OutFormat == SMM_File_Type_MPEG2_TS || m_OutFormat == SMM_File_Type_MP4 || m_OutFormat == SMM_File_Type_MKV )
	{
		...
		if( pin != NULL )
		{
			...
			if( FAILED( hr ) )
			{
				CExMsg err(_T("Connecting audio Trimmer pinn with Muxer has been failed"), hr);
				err.MsgBox();
				return hr;
			}
		}
		...
		if( m_OutFormat == SMM_File_Type_MPEG2_VES )
		{
...
HRESULT  CTrimGrpah::OnInitCompleted()
{
	...
	if (m_OutFormat == SMM_File_Type_MPEG2_VES ) // otherwise engine uses out type equal to input one
	{
		var =  (long) m_OutFormat;
	...
	if( m_OutFormat == SMM_File_Type_MPEG2_TS )
	{
		ASSERT( m_pConfigMuxer != NULL );
		var =  RESULT_STREAM_PROGRAM;
		hr = m_pConfigMuxer->SetValue(&ElMux_OutputStreamType ,&var);
...
HRESULT  CTrimGrpah::OnInitCompletedGOPAcc()
{
	...
	ASSERT( m_pConfigMuxer != NULL && m_OutFormat == SMM_File_Type_MPEG2_VES );
...
{
#define SELF_RELEASE(point) if( point ) {point.Release(); point = NULL;}
...
REFERENCE_TIME CTrimGrpah::GetMuxerPosition()
{
	...
	else if( m_OutFormat == SMM_File_Type_MP4 )
	{
		hr = m_pConfigMuxer->GetValue(&SMMP4M_MUX_TIME, &val);
	}
	else if( m_OutFormat == SMM_File_Type_MKV )
	{
		hr = m_pConfigMuxer->GetValue(&SMMKVM_MUXTIME, &val);
	}
