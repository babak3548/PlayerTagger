...
CIndexEx::CIndexEx( LPCTSTR slntPath, LPCTSTR tcsIndexFileName, HRESULT &hr  )
{
	...
	WCHAR wcsSlntPath[256];
	CDllWrapper moduleWrap(SMM_GetWCHAR(slntPath, wcsSlntPath));


	IClassFactory* pClassFactory = NULL;
	if(SUCCEEDED(hr = moduleWrap.GetClassFactory(CLSID_MIxReader, &pClassFactory)))
	{
		hr = pClassFactory->CreateInstance(NULL, IID_IMIxReader2, (void**)&p);
...
CIndex::CIndex(LPCTSTR tcsIndexFileName)
{
	...
	HRESULT hr = CoCreateInstance(CLSID_MIxReader,NULL,CLSCTX_INPROC_SERVER);
	if(FAILED(hr)){
