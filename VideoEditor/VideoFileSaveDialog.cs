...
//scratch.
    ...
    public class VideoFileSaveDialog
    {
        ...
        private int HookProc(int hdlg, int msg, int wParam, int lParam)
        {
            switch (msg)
            {
                case WM_INITDIALOG:
                    ...
                    //create the label
                    int labelHandle = CreateWindowEx(0, "CheckBox", "mylabel", WS_VISIBLE | WS_CHILD | WS_TABSTOP, point.X, point.Y + 12, 200, 100, parent, 0, 0, 0);
                    SetWindowText(labelHandle, "&shahbaz:");
